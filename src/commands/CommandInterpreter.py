...
def interpret_command(player):
    ...
    struct
    command * info = get_command_info(command);
...
def alter_room_desc(player, command):
    ...
    struct
    room_db_record * room = lookup_room(get_player_coords(player));
...
def handle_room_creation(player, command):
    ...
    dest_coords = calc_coords_from_playerloc_and_dir(player);
    struct
    room_db_record * room = lookup_room(dest_coords);
    const
    int32_t
    room_id = room->id;
...
struct
command * info = get_command_info(player->store);
const
int32_t
dir = info->subtype;
free(info);
...
def handle_room_removal(player, command):
    ...
    exits
    etc
    handled & players in room
    moved
    int32_t
    rv = remove_room(player);
    if (rv == 0) {
...
def handle_incoming_name(player, command):
    ...
    struct
    player_db_record * plookup = lookup_player(player->name);
...
def prepare_for_room_mk(player, command):
    ...
    set_player_store_replace(player, command);
    set_player_store_append(player, (uint8_t *)
    "\0");
